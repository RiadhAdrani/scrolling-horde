...
setup:
	@npm i -g dotenv-cli
	@npm i -g husky && husky
# @npm exec --prefix ./apps/web playwright@next install-deps
# @npm exec --prefix ./apps/web playwright@next install

install:
		${USER} npm i
		${USER} npm --prefix ./apps/api install
		${USER} npm --prefix ./apps/web install
...
clean:
		...
		${USER} sudo rm -rf ./apps/web/node_modules
...
test-e2e:
	${USER} dotenv ${TEST_ENV} -c -- npm --prefix apps/web run test:e2e

test-e2e-ui:
	${USER} dotenv ${TEST_ENV} -c -- npm --prefix apps/web run test:e2e:ui
...
check-web-lint:
	npm --prefix apps/web run lint:check
...
check:
	...
	make check-web-lint
	make check-web-types
	make check-web-format
